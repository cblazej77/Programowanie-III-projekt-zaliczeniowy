...
public class Querries {
    ...
    public List<LekcjeEntity> findLekcjeForPrzedmiotByUserLogin(String login, String dzien) {
        EntityManager entityManager = FACTORY.createEntityManager();
        Query query = (Query) entityManager.createQuery("SELECT l.godzina, np.przedmiotyByIdp.nazwa FROM LekcjeEntity l JOIN l.nauczycieleprzedmiotowByIdnp np JOIN l.klasyByKlasa k JOIN k.uczniowiesByIdk u JOIN u.uzytkownicyByIdus us WHERE us.login = :login AND l.dzien = :dzien ORDER BY l.godzina");
        query.setParameter("login", login);
